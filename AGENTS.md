...
- Never trust the status for feasibility either. Stopped by the clock inside the root heuristics, CBC has reported `Optimal (within gap tolerance)` with the best objective in its log and written a solution file that breaks the balance rows by whole charging steps, flagged `**` in the file, which pulp strips. Every stage that keeps a solver result runs `_is_feasible()`, `problem.valid(FEASIBILITY_TOLERANCE)`, before doing so, and `solve()` checks once more before reporting. A new stage that keeps a result must do the same.
